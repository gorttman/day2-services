...
# Filesystem NAME_MAX is 255 bytes. This budget is kept well under that
# because the resulting name also gets combined with two sidecar
# affixes elsewhere: ".incoming-" (10 bytes, quarantine()'s temp-copy
# prefix) and ".quarantine-reason.txt" (22 bytes, the reason sidecar
# suffix) - either of those pushes the real ceiling down to ~233-245
# bytes, so 200 leaves comfortable headroom for both.
MAX_QUARANTINE_NAME_BYTES = 200


def _byte_truncate(s, max_bytes):
    if max_bytes <= 0:
        return ""
    b = s.encode("utf-8")
    if len(b) <= max_bytes:
...
def _quarantine_target_name(path, quarantine_dir, exists_fn):
    """Builds a parent-disambiguated quarantine filename, truncated to
    fit the filesystem's NAME_MAX. Found live 2026-07-22: long book
    titles combined with the parent-dir disambiguation prefix (added to
    fix basename-only collisions, see below) routinely exceeded 255
    bytes, so every long-titled file failed to quarantine with
    "[Errno 36] File name too long" on every single run, forever - the
    parent-prefix fix for one collision bug silently introduced this
    one. Disambiguates by parent directory, not just basename - generic
    ...
    genuinely different files share both parent dir and filename, so a
    source is never silently left unquarantined for lack of a slot.
    Truncates the stem (the original filename) rather than the
    extension, since ebook-meta/calibre still need a recognizable
    extension to identify the format; truncates parent too as a
    fallback so this can never loop forever regardless of how long a
    parent directory name is."""
    name = os.path.basename(path)
    parent = os.path.basename(os.path.dirname(path)) or "root"
    stem, ext = os.path.splitext(name)
    parent = _byte_truncate(parent, 80)
    parent_bytes = len(parent.encode("utf-8"))
    ext_bytes = len(ext.encode("utf-8"))

    def build(suffix=""):
        suffix_bytes = len(suffix.encode("utf-8"))
        stem_budget = MAX_QUARANTINE_NAME_BYTES - parent_bytes - len(" - ") - ext_bytes - suffix_bytes
        fitted_stem = _byte_truncate(stem, stem_budget)
        return f"{parent} - {fitted_stem}{ext}{suffix}"

    disambiguated = build()
    final_path = os.path.join(quarantine_dir, disambiguated)
    suffix_n = 2
    while exists_fn(final_path):
        disambiguated = build(suffix=f" ({suffix_n})")
        final_path = os.path.join(quarantine_dir, disambiguated)
        suffix_n += 1
    return disambiguated, final_path
...
def quarantine(path, reason, quarantine_dir, counters):
    ...
    name, final_path = _quarantine_target_name(path, quarantine_dir, os.path.exists)
...
def quarantine_broken_symlink(path, reason, quarantine_dir, counters):
    ...
    name, final_path = _quarantine_target_name(path, quarantine_dir, os.path.lexists)
